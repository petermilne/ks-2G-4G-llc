...
/** @file rtm-t-feedback.cpp demonstrates llcontrol with ACQ196/RTM-T.
 * llcontrol with rtm-t is a simplified version of ACQ196/SYNC2V.
 - USAGE: rmt-t-feedback AISLOT AOSLOT
 - Outputs 2 channel AWG on AO01, AO02.
...
#define TLATCH_OFFSET ((NCHAN/2) + LLCV2_STATUS_TLATCH)	/* longwords */


static RTM_T_Device *ai_dev;
static RTM_T_Device *ao_dev;
...
struct AO_LLC_DEF aollc_def = {
	32 * sizeof(u32) + 2 * sizeof(u32),
	RTM_T_USE_HOSTBUF
};
/*
...
void cpy32(volatile u32* dst, volatile u32* src, int nwords)
{
	while(nwords--){
		*dst++ = *src++;
	}
...
int llcontrol() {
	...
	volatile u32 *ao_hb = (volatile u32*)ao_dev->getHostBufferMapping();
	u32 tlatches[3];
...
#define tlatch2 tlatches[2]     // raw tlatch detects change of sample
	...
	int rc = ioctl(ao_dev->getDeviceHandle(), RTM_T_START_AOLLC, &aollc_def);
	...
	rc = ioctl(ai_dev->getDeviceHandle(), RTM_T_START_LLC, &llc_def);
	dbg(1, "ioctl complete ..");
	if (rc != 0){
		err("ioctl RTM_T_START_LLC failed %d", rc);
		_exit(-rc);
	...
	for (int sample = 0; sample < samples; ++sample){
		...
		if (CH_COPY){
			cpy32(ao_hb, hb, CH_COPY);
		}else{
			cpyfill(ao_hb, hb[0], 16);
		}

		if (feed_forward){
		/* feedforward waveform */
			ao_hb[0] = awg[(sample+1)%nawg];
		}
...

{
	...
	while(fgets(buf, 128, fp)){
		...
		if (sscanf(buf, "[%2d] %x %x", &ix, &va, &pa) == 3){
			if (ix == 0){
				llc_def.target_pa = aollc_def.src_pa = pa;
...
static void init_defaults(int argc, char* argv[])
{
	...
	if (argc < 3){
		fprintf(stderr, "ERROR: usage rtm-t-feedback AI AO");
	}
	int ai_devnum = atoi(argv[1]);
	int ao_devnum = atoi(argv[2]);

	assert(IN_RANGE(ai_devnum, 100, 110));
	assert(IN_RANGE(ao_devnum, 100, 110));
	assert(ai_devnum != ao_devnum);

	ai_dev = new RTM_T_Device(ai_devnum, nbuffers);
	ao_dev = new RTM_T_Device(ao_devnum, nbuffers);
	...
	if (getenv("CH_COPY_PAIRS")){
		CH_COPY = atoi(getenv("CH_COPY_PAIRS"));
		printf("CH_COPY set %d\n", CH_COPY);
	}
